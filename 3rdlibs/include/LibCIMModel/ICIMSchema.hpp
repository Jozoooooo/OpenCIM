...
namespace NNU::OpenCIM {
    struct ICIMSchema {
        ...
        [[maybe_unused]] virtual ICIMSchema *getIncludeCIMStandard(int index) = 0;
    ...
    extern "C" [[maybe_unused]]  CIM_MODEL_API ICIMSchema *CreateCIMSchema(); // 创建CIM模式层接口

    extern "C" [[maybe_unused]]  CIM_MODEL_API void DestroyCIMSchema(ICIMSchema *iCIMSchema); // 销毁场景标准接口
}