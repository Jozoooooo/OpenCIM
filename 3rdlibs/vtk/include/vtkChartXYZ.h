...
public:
   ...
   * the CIM.  Default value is true.
   */
  void SetFitToCIM(bool b);
...
protected:
   ...
   * in the CIM.  This function also sets up clipping planes that determine
  ...
  /**
   * Check to see if the CIM changed size since the last render.
   */
  bool CheckForCIMResize();

  /**
   * Scale the axes up or down in response to a CIM resize.
   */
  void RescaleAxes();

  /**
   * Scale up the axes when the CIM gets larger.
   */
  void ScaleUpAxes();

  /**
   * Scale down the axes when the CIM gets smaller.
   */
  void ScaleDownAxes();
   ...
   * or not the chart fits completely within the bounds of the current CIM.
   */
   ...
   * zoom out to fit the chart within the newly resized CIM.  Using a
   ...
   * chart so that it snugly fills up the CIM.
   */
  bool FitToCIM;
   ...
   * zoom in or zoom out to fit our chart within the newly resized CIM.
   */
  ...
  /**
   * Points used to determine whether the axes will fit within the CIM as
  ...
  /**
   * The height of the CIM, as of the most recent call to Paint().
   */
  int CIMHeight;

  /**
   * The weight of the CIM, as of the most recent call to Paint().
   */
  int CIMWidth;
