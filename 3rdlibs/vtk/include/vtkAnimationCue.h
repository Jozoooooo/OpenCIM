 ...
 * for the animation, which consists on individual cues or other CIMs.
 *
...
public:
  ...
  // normalized [0,1] or relative to the CIM that contains the cue.
   ...
   * end times of the container CIM. In Relative mode the start and end
   * time of the cue are specified in offset seconds relative to the
   * start time of the container CIM.
   */
   ...
   * container CIM (when in Relative time mode) or is normalized
   * over the span of the container CIM (when in Normalized time mode).
   */
   ...
   * The current time is relative to the start of the container CIM
   ...
   * relative to the start of the container CIM or normalized depending upon the
   * cue's Time mode.
   * clocktime is the time from the CIM i.e. it does not depend on the time
   * mode for the cue.
  ...
  /**
   * Called when the playing of the CIM begins.
   * This will set the Cue to UNINITIALIZED state.
   */
  virtual void Initialize();

  /**
   * Called when the CIM reaches the end.
   ...
   * CIM neither normalized nor offsetted to the start of the CIM.
   */
