...
namespace NNU::OpenCIM {
    struct ICIMContent {
        ...
        [[maybe_unused]] virtual ICIMContent *getIncludeCIMContent(int index) = 0; // 根据索引获取引用的外部场景内容接口
    };

    extern "C" [[maybe_unused]]  CIM_MODEL_API ICIMContent *CreateCIMContent(); // 创建场景内容接口

    extern "C" [[maybe_unused]]  CIM_MODEL_API void DestroyCIMContent(ICIMContent *iCIMContent); // 销毁场景模型接口
}
